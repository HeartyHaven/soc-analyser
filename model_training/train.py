...
def checkpoint(logger, model, epoch, save_path,min_loss):
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    model_out_path = f"./{save_path}/model_iters_{epoch}_loss_{min_loss}.pth"
...
def train():
    ...
    # optimizer = optim.SGD(model.parameters(), lr=arg_dict['lr'], momentum=0.9)
    # Build lr scheduler
    ...
    # save_freq = int(arg_dict['max_iters']/50)
    min_loss=1000
    min_loss = float('inf')  # 初始化最小损失值为正无穷大

    while iter_num < arg_dict['max_iters']:
        with tqdm(total=print_freq) as bar:
            for feature, label, _ in dataset:  # 每次一个batch的大小
                # if arg_dict['cpu']:
        ...
        if  iter_num > 10000:  # 当训练轮数超过15000轮并且达到保存频率时执行保存模型
            if epoch_loss / print_freq < min_loss:  # 如果当前epoch的总损失值比最小损失值小，则更新最小损失值
                min_loss = epoch_loss / print_freq
                checkpoint(logger, model, iter_num, log_dir,min_loss)
